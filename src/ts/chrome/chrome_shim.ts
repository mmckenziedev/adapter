...
import shimGetUserMedia = require("./getusermedia");

export { shimGetUserMedia };
  ...
  if (!("getStats" in window.RTCRtpSender.prototype)) {
    ...
    window.RTCRtpSender.prototype.getStats = function() {
      ...
      return this._pc
        .getStats()
        .then((
          result /* Note: this will include stats of all senders that
...
export function shimAddTrackRemoveTrackWithNative(window) {
  ...
  window.RTCPeerConnection.prototype.getLocalStreams = function() {
    ...
    return Object.keys(this._shimmedLocalStreams).map(
      streamId => pc._shimmedLocalStreams[streamId][0]
    );
  };
...
export function shimAddTrackRemoveTrack(window) {
  ...
  window.RTCPeerConnection.prototype.removeTrack = function(sender) {
    ...
    Object.keys(pc._streams).forEach(streamid => {
      const hasTrack = pc._streams[streamid]
        .getTracks()
